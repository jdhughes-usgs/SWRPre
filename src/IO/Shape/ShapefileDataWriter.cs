...
{
    /// <summary>
    /// Writes a complete shapefile structure (shp, shx and dbf).
    /// </summary>
    public class ShapefileDataWriter
    {
        ...
        public void Write(IList featureCollection)
        {
            ...
            try
            {
                // Test if the Header is initialized. If not, then create the header
                // from the first feature in the collection if it is not empty.
                if (Header == null)
                {
                    if (featureCollection.Count > 0)
                        Header = GetHeader( (Feature) featureCollection[0], featureCollection.Count);
