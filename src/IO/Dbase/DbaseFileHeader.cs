...
namespace SWRPre.IO
{
	...
	{        
        ...
        public void BuildHeader(Shape2D shape)
        {
            
            for (int i = 0; i < shape.NumAttributes; i++)
            {
                ShapeAttribute attrib = shape.GetAttribute(i);
                Type type = attrib.Value.GetType();
                char ctype = DbaseFieldDescriptor.GetDbaseType(type); 
                int length = DbaseFieldDescriptor.GetDbaseLength(type);
                int decimals = DbaseFieldDescriptor.GetDbaseDecimals(type);
                AddColumn(attrib.Name,ctype,length,decimals);
            }
            return;
        }
